// Hero with form - Updated October 6, 2024
function noop() { }
const identity = x => x;
...
function create_if_block_3(ctx) {
	...
	return {
		...
		h() {
			attr(div, "class", "message error svelte-1fvdocl");
		},
		m(target, anchor) {
...
function create_if_block_2(ctx) {
	...
	return {
		...
		h() {
			attr(div, "class", "message svelte-1fvdocl");
		},
		m(target, anchor) {
			insert_hydration(target, div, anchor);
			append_hydration(div, t);
...
function create_if_block(ctx) {
	...
	return {
		...
		h() {
			...
			attr(form_1, "class", "svelte-1fvdocl");
		},
		m(target, anchor) {
			...
			if (!mounted) {
				dispose = listen(form_1, "submit", prevent_default(/*submit_form*/ ctx[5]));
				mounted = true;
			}
		},
		p(ctx, dirty) {
...
function create_fragment(ctx) {
	...
	function select_block_type(ctx, dirty) {
		if (!/*submitted*/ ctx[3] && !/*error*/ ctx[4]) return 0;
		if (/*submitted*/ ctx[3]) return 1;
		if (/*error*/ ctx[4]) return 2;
		return -1;
	}
	...
	return {
		...
		h() {
			attr(h1, "class", "headline svelte-1fvdocl");
			attr(section, "class", "section-container svelte-1fvdocl");
		},
		m(target, anchor) {
...
function instance($$self, $$props, $$invalidate) {
	...
	$$self.$$set = $$props => {
		...
		if ('graphics' in $$props) $$invalidate(7, graphics = $$props.graphics);
	};

	return [form, heading, submitting, submitted, error, submit_form, props, graphics];
}
